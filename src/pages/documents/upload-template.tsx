    ...
    <DashboardLayout>
      <Header>
        <Breadcrumb>
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink href={ROUTES.uploadDocumentTemplate}>
                Upload Template
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Employee Contract</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      ...
      <Main>
        <section className="min-h-screen h-full flex items-start gap-7 w-full bg-periwinkle-25 pl-7">
          {!isDocumentReady ? (
            ...
            <div
              ...
              className="w-[72%] mt-6 bg-periwinkle-100 border border-neutral-50 rounded-xl rounded-b-none relative"
            >
              <PdfDocument
                document={document}
                droppedItems={droppedItems}
          ...
          <div className="w-[28%] sticky top-18 border-l border-b border-periwinkle-100 bg-white grid grid-cols-2 text-neutral-600 text-center">
            {draggableItems.map(item => (
              <DraggableComponent
                key={`${item.id}-${uuidv4()}`}
                id={`${item.id}-${uuidv4()}`}
              >
                <div className="flex flex-col items-center gap-1.5 py-10 border-r border-b border-periwinkle-100">
                  <img
